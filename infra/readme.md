...
func init() {
	hive.Prepare(func(initiator hive.Initiator) {
		...
		*/
		initiator.InjectController(func(ctx hive.Context) (com *Single) {
...
func (goods *GoodsController) PutStock() hive.Result {
	var request struct {
...
func init() {
	...
	hive.Prepare(func(initiator hive.Initiator) {
		...
		initiator.InjectController(func(ctx hive.Context) (com *JSONRequest) {
			//从Infra池里取出注入到控制器。
			initiator.FetchInfra(ctx, &com)
			return
		})
...
type JSONRequest struct {
	hive.Infra	//多例需继承hive.Infra
}

// BeginRequest 每一个请求只会触发一次
func (req *JSONRequest) BeginRequest(worker hive.Worker) {
	// 调用基类初始化请求运行时
