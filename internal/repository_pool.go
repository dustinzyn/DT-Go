...
// app .
func (pool *RepositoryPool) app() *Application {
	if pool.private {
		return privateApp
	} else {
		return publicApp
...
func (pool *RepositoryPool) get(t reflect.Type) (ok bool, result reflect.Value) {
	...
	if len(values) == 0 {
		panic(fmt.Sprintf("BindRepository: func return to empty, %v", reflect.TypeOf(fun)))
	}
...
func (pool *RepositoryPool) diRepoFromValue(value reflect.Value, instance *serviceElement) bool {
	...
	if value.Kind() == reflect.Ptr && value.IsZero() {
		...
		if !value.CanSet() {
			panic(fmt.Sprintf("This use repository object must be a capital variable: %v" + value.Type().String()))
		}
		//创建实例并且注入基础设施组件
		value.Set(newfield)
		allFieldsFromValue(newfield, func(repoValue reflect.Value) {
			pool.app().comPool.diInfraFromValue(repoValue)
			if repoValue.IsNil() {
				return
			}
