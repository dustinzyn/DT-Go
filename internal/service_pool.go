...
func newServicePool(private bool) *ServicePool {
...
// app .
func (pool *ServicePool) app() *Application {
	if pool.private {
		return privateApp
	} else {
		return publicApp
...
func (pool *ServicePool) get(rt *worker, service interface{}) {
	...
	if svalue.Kind() != reflect.Ptr {
		panic(fmt.Sprintf("No dependency injection was found for the service object,%v", svalue.Type()))
	}
	ptr := svalue.Elem()
	...
	if newService == nil {
		panic(fmt.Sprintf("No dependency injection was found for the service object,%v", ptr.Type()))
	}
...
func (pool *ServicePool) create(rt Worker, service reflect.Type) interface{} {
	...
	if newService == nil {
		panic(fmt.Sprintf("No dependency injection was found for the service object,%v", service))
	}
...
func (pool *ServicePool) bind(t reflect.Type, f interface{}) {
	pool.pool[t] = &sync.Pool{
		New: func() interface{} {
			...
			if len(values) == 0 {
				panic(fmt.Sprintf("BindService: func return to empty, %v", reflect.TypeOf(f)))
			}
			...
			allFields(newService, func(fieldValue reflect.Value) {
				...
				pool.app().repoPool.diRepoFromValue(fieldValue, &result)
				pool.app().comPool.diInfraFromValue(fieldValue)
				pool.app().factoryPool.diFactoryFromValue(fieldValue, &result)
...
func (pool *ServicePool) malloc(t reflect.Type) interface{} {
	...
	if newService == nil {
		panic(fmt.Sprintf("BindService: func return to empty, %v", t))
	}
	return newService
}
